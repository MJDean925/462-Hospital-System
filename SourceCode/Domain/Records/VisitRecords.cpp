...
namespace Domain::Records{
    ...
    void VisitRecords::createNew(std::string pName, std::string dName, std::string pTestimony, std::string diag, std::string treatment, std::string ref, std::string presc, tm inDate, tm outDate, std::string output){
        TechnicalServices::Persistence::VisitRecord v = {pName, dName, inDate, outDate, pTestimony, diag, treatment, ref, presc};
    ...
    std::vector<TechnicalServices::Persistence::VisitRecord> VisitRecords::getRecordsByRange(int beginMonth, int beginYear, int endMonth, int endYear) {
        std::vector<TechnicalServices::Persistence::VisitRecord> allRecords =  _persistentData->getAllVisitRecords();

        tm beginDate = { 0, 0, 0, 0, beginMonth - 1, beginYear - 1900, 0, 0, 0 };
        tm endDate = { 0, 0, 0, 0, endMonth - 1, endYear - 1900, 0, 0, 0 };

        std::vector<TechnicalServices::Persistence::VisitRecord> records;
        std::copy_if(allRecords.begin(), allRecords.end(), std::back_inserter(records), [beginDate, endDate](TechnicalServices::Persistence::VisitRecord r) mutable {
            return ( (difftime(mktime(&endDate), mktime(&r.inDate)) > 0.0) && (difftime(mktime(&r.inDate), mktime(&beginDate)) > 0.0) );
        });
        return records;
    }

    std::vector<TechnicalServices::Persistence::VisitRecord> VisitRecords::getRecords(int size) {
        return _persistentData->getRecentVisitRecords(size);
    }

    std::vector<TechnicalServices::Persistence::VisitRecord> VisitRecords::getVRecords(){
        return _persistentData->getAllVisitRecords();
    }
}